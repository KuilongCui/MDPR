...
class PartSbaseline(nn.Module):
    ...
    def __init__(
        ...
        if last_stride == 1:
            self.upSamper3 = Embedding(self.num_attn, 1, self.num_attn, with_ibn=with_ibn, norm_type=norm_type, bias=True)
        else:
        ...
        self.embedding4 = Embedding(feat_dim, 1, embedding_dim, with_ibn=with_ibn, norm_type=norm_type)
        self.embedding3 = Embedding(feat_dim, num_head_chunk, embedding_dim, with_ibn=with_ibn, norm_type=norm_type)
        self.embedding2 = Embedding(feat_dim, num_head_chunk, embedding_dim, with_ibn=with_ibn, norm_type=norm_type)
        self.embedding1 = Embedding(feat_dim, num_head_chunk, embedding_dim, with_ibn=with_ibn, norm_type=norm_type)
        ...
        self.num_head_chunk = num_head_chunk

        self.chunk_heads1 = nn.ModuleList([self.head1])
        for i in range(1, self.num_head_chunk):
            self.chunk_heads1.append(copy.deepcopy(self.head1))

        self.chunk_heads2 = nn.ModuleList([self.head2])
        for i in range(1, self.num_head_chunk):
            self.chunk_heads2.append(copy.deepcopy(self.head2))
        
        self.chunk_heads3 = nn.ModuleList([self.head3])
        for i in range(1, self.num_head_chunk):
            self.chunk_heads3.append(copy.deepcopy(self.head3))
    ...
    def from_config(cls, cfg):
        ...
        return {
            'num_head_chunk': cfg.MODEL.HEADS.NUM_CHUNK,
    ...
    def forward(self, batched_inputs):
        ...
        chunk_f3 = torch.chunk(feature3, self.num_head_chunk, dim=2)
        chunk_f2 = torch.chunk(feature2, self.num_head_chunk, dim=2)
        chunk_f1 = torch.chunk(feature1, self.num_head_chunk, dim=2)
        
        feature4 = self.embedding4(feature4)
        chunk_f3 = self.embedding3(chunk_f3)
        chunk_f2 = self.embedding2(chunk_f2)
        chunk_f1 = self.embedding1(chunk_f1)
        ...
        if self.training:
            ...
            outputs3 = []
            for i in range(self.num_head_chunk):
                outputs3.append(self.chunk_heads3[i](chunk_f3[i], targets))

            outputs2 = []
            for i in range(self.num_head_chunk):
                outputs2.append(self.chunk_heads2[i](chunk_f2[i], targets))

            outputs1 = []
            for i in range(self.num_head_chunk):
                outputs1.append(self.chunk_heads1[i](chunk_f1[i], targets))

            attn_output = self.attn_head(attn_feat, targets, batched_inputs=batched_inputs)
            
            losses_main = self.losses_main(attn_output, outputs4, targets, "main")
            losses_branch = self.losses_branch(outputs3, outputs2, outputs1, targets, "branch")

            total_losses = {}
            for k,v in losses_main.items():
                total_losses[k] = v
            for k,v in losses_branch.items():
                total_losses[k] = v
            ...
            outputs3 = []
            for i in range(self.num_head_chunk):
                outputs3.append(self.chunk_heads3[i](chunk_f3[i]))
            outputs3 = torch.cat(outputs3, dim=1)

            outputs2 = []
            for i in range(self.num_head_chunk):
                outputs2.append(self.chunk_heads2[i](chunk_f2[i]))
            outputs2 = torch.cat(outputs2, dim=1)

            outputs1 = []
            for i in range(self.num_head_chunk):
                outputs1.append(self.chunk_heads1[i](chunk_f1[i]))
    ...
    def losses_branch(self, b3_outputs, b2_outputs, b1_outputs, gt_labels, prefix=""):
        """
        ...
        if 'CrossEntropyLoss' in loss_names:
            ...
            for i in range(self.num_head_chunk):
                loss_dict[prefix + 'cls_b1_'+str(i)] = cross_entropy_loss(
                    b1_outputs[i]['cls_outputs'],
                    gt_labels,
                    ce_kwargs.get('eps'),
                    ce_kwargs.get('alpha')
            ...
            for i in range(self.num_head_chunk):
                loss_dict[prefix + 'cls_b2'+str(i)] = cross_entropy_loss(
                    b2_outputs[i]['cls_outputs'],
                    gt_labels,
                    ce_kwargs.get('eps'),
                    ce_kwargs.get('alpha')
                ) * ce_kwargs.get('scale')
            ...
            for i in range(self.num_head_chunk):
                loss_dict[prefix + 'cls_b3'+str(i)] = cross_entropy_loss(
                    b3_outputs[i]['cls_outputs'],
                    gt_labels,
                    ce_kwargs.get('eps'),
                    ce_kwargs.get('alpha')
                ) * ce_kwargs.get('scale')
        ...
        if 'TripletLoss' in loss_names:
            ...
            for i in range(self.num_head_chunk):
                loss_dict[prefix + 'triplet_b1_'+str(i)] = triplet_loss(
                    b1_outputs[i]['features'],
                    gt_labels,
            ...
            for i in range(self.num_head_chunk):
                loss_dict[prefix + 'triplet_b2_'+str(i)] = triplet_loss(
                    b2_outputs[i]['features'],
                    gt_labels,
                    tri_kwargs.get('margin'),
                    tri_kwargs.get('norm_feat'),
                    tri_kwargs.get('hard_mining')
            ...
            for i in range(self.num_head_chunk):
                loss_dict[prefix + 'triplet_b3_'+str(i)] = triplet_loss(
                    b3_outputs[i]['features'],
                    gt_labels,
                    tri_kwargs.get('margin'),
                    tri_kwargs.get('norm_feat'),
                    tri_kwargs.get('hard_mining')
    ...
    def losses_main(self, attn_output, b4_outputs, gt_labels, prefix=""):
        """
        Compute loss from modeling's outputs, the loss function input arguments
        must be the same as the outputs of the model forwarding.
        """
...
class Embedding(nn.Module):
    def __init__(self, input_dim, num_chunk, out_dim, with_ibn, norm_type, bias= False):
        ...
        if self.num_chunk > 1:
            ...
            for i in range(num_chunk):
                conv = nn.Conv2d(input_dim, out_dim, 1, 1, bias=bias)
                conv.apply(weights_init_kaiming)
                self.embedding.append(conv)

                if with_ibn:
                    bn = IBN(out_dim, norm_type)
                else:
                    bn = get_norm(norm_type, out_dim)
