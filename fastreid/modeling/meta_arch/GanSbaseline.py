...
class GanSbaseline(nn.Module):
    ...
    def __init__(
        ...
        self.gan1 = GraphConvolution(2, feat_dim//8, feat_dim//8, feat_dim//8)
        self.gan2 = GraphConvolution(3, feat_dim//4, feat_dim//4, feat_dim//4)
        self.gan3 = GraphConvolution(4, feat_dim//2, feat_dim//2, feat_dim//2)
        # self.gan4 = GraphConvolution(4, feat_dim, feat_dim, feat_dim)
    ...
    def from_config(cls, cfg):
        ...
        backbone2 = nn.ModuleList([
            copy.deepcopy(all_blocks.layer3),
            copy.deepcopy(all_blocks.layer4)
        ])
        backbone3 = nn.ModuleList([
            copy.deepcopy(all_blocks.layer4)
        ])
    ...
    def forward(self, batched_inputs):
        ...
        if self.detach_attn or self.detach_all:
        ...
        # layer 1
        f1_all = torch.stack((f1, x1), dim=1)
        f1_stack = self.gan1(f1_all)
        f11 = f1_stack[:, 0] + f1
        
        f12 = self.backbone1[0](f11)

        # layer 2
        f2_all = torch.stack((f12, f2, x2), dim=1)
        f2_stack = self.gan2(f2_all)
        f12 = f2_stack[:, 0] + f12
        f22 = f2_stack[:, 1] + f2

        f13 = self.backbone1[1](f12)
        f23 = self.backbone2[0](f22)

        # layer 3
        f3_all = torch.stack((f13, f23, f3, x3),dim=1)
        f3_stack = self.gan3(f3_all)
        f13 = f3_stack[:,0] + f13
        f23 = f3_stack[:,1] + f23
        f33 = f3_stack[:,2] + f3

        f14 = self.backbone1[2](f13)
        f24 = self.backbone2[1](f23)
        f34 = self.backbone3[0](f33)
...
class GraphConvolution(nn.Module):
    def __init__(self, in_num, in_features, out_features, n_hidden, bias=False):
        ...
        for i in range(in_num):
            embedding = nn.Conv2d(in_features, n_hidden, 1, 1, bias=bias)
            embedding.apply(weights_init_kaiming)
            self.encode.append(embedding)
        
        self.activation = nn.ReLU()

        self.bnneck = nn.Conv2d(n_hidden * in_num, n_hidden * in_num, 1, 1, bias=bias)
        self.bnneck.apply(weights_init_kaiming)

        self.decode = nn.Conv2d(n_hidden * in_num, out_features * (in_num-1), 1, 1, bias=bias)
        self.decode.apply(weights_init_kaiming)
    ...
    def forward(self, input):
        ...
        for i in range(self.in_num):
            embedding_input.append(self.encode[i](input[:, i]))

        neck = torch.cat(embedding_input, dim=1).view(B, self.in_num * self.n_hidden, H, W)

        neck = self.bnneck(neck)

        out = self.decode(neck).view(B, self.in_num-1, self.out_features, H, W)
